% Structural identifiability analysis using the sensitivity matrix. Series 2CXM, single-echo scan.
...
[Mt_L,Mp_L,Me_L] = my_solve_series_2CXM_numerical(c,TA,tau,R1p,R1e,f,(d),M0,t_array,heaviside_approx);
    ...
    run('literature_vals.m')
     ...
     [f,TA,R1p,R1e,R2p,R2e,d]=fixparameters(f,TA,R1p,R1e,R2p,R2e,d);
    [Mt_R,Mp_R,Me_R] = my_solve_series_2CXM_numerical(c,TA,tau,R1p,R1e,f,(d),M0,t_array,heaviside_approx);
...
xticks([1:1:length(p)])
%xticklabels(string(p))
xticklabels(["CBF", "ATT", "R1_b", "R1_t", "\delta_t"]);
...
%[M0 tau F TA R1p R1e R2p R2e] = [1 1000 48*(1/100)*(1/60) 1570) 1/1650 1/1330 1/110 1/77]
% R1p  = 1/1.650;
% R1e = 1/1.330;
% f=48*(1/100)*(1/60);
% R2p=1/0.110;
