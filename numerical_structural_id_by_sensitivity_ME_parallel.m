...
% Parallel 2 compartment model, multi-echo scan.
...
[Mt_L,Mp_L,Me_L,Mt0_L] = my_solve_parallel_2CXM_T2_numerical(c,TA,tau,R1p,R1e,R2p,R2e,f,kw,M0,t_array,TI,heaviside_approx);
    t=t_array;
    M(:,1,k)=Mt_L;
    ...
    run('literature_vals.m')
     ...
     [f,TA,R1p,R1e,R2p,R2e,kw]=fixparameters(f,TA,R1p,R1e,R2p,R2e,kw);
    [Mt_R,Mp_R,Me_R,Mt0_R] = my_solve_parallel_2CXM_T2_numerical(c,TA,tau,R1p,R1e,R2p,R2e,f,kw,M0,t_array,TI,heaviside_approx);
...
%L2=legend(h2,["CBF", "ATT", "R1_b", "R1_t", "R2_b", "R2_t", "k_w"],'Location','northeastoutside');
...
subplot(2,1,1)
pEVs=semilogy(1:1:length(singular_vals),singular_vals,'o','Linewidth',alw);
...
xticks([1:1:length(p)])
%xticklabels(string(p))
xticklabels(["CBF", "ATT", "R1_b", "R1_t", "R2_b", "R2_t", "k_w"]);
xlim([1 7]);
